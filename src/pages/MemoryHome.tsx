import { Box, Button, Card, CardBody, Flex, Heading, Link, Stack, Tag, Text } from "@chakra-ui/react";
  ...
  useEffect(() => {
    const getUserRecord = async () =>{
      try {
        ...
        const newlatestStories = newStories.slice(0, 2);
        setLatestStories(newlatestStories); // 最新の3件を取得
        setStories(newStories);
        setRandomStory(getRandomStory(newStories.filter(story => !newlatestStories.includes(story))));
        setUser(newUser);
      } catch (error){
